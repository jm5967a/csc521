...
def Program(index, function):
    ...
    if (len(
            final) < 2):  # if the final arrays contains more than
        # substrings it will be program0 and if not it will be program1
        done = ['Program1']
    else:
        done = ['Program0']
    while x < len(
            final):  # loop to get the correct formatting
        # for the parser--Append to final array
        if x == 0:
            done.append(final[x])
        else:
...
def Statement(returned_index,
              function):  # main function to break off to specific functions
    # --Function Declaration,assignment, print
    global final
        ...
        if success:
            if function != 1:
                final.append(["Statement0",
                              returned_subtree])  # primary code to add
                #  tree to final array
                returned_index += 1
            else:
                temp.append(
...
def Param(token_index):  # function that includes parameter and paramerterlist
    ...
    def Parameter(token_index):
        ...
        (success, returned_index, returned_subtree) = Expression(
            token_index)  # try to run token string through expression
        # function and return tree
        if success:
            subtree = ['Parameter0', returned_subtree]
            return [True, returned_index, subtree]
        (success, returned_index, returned_subtree) = Name(
            token_index)  # try to run token string through name
        #  function and return tree
        if success:
    ...
    (success, returned_index, returned_subtree) = Parameter(
        token_index)  # try to run token string through
    # parameter function and return tree
    if success:
        subtree = returned_subtree
        if 'COMMA' == tokens[
            returned_index]:  # check if index is comma and if so
            # increase index
            while tokens[returned_index] == 'COMMA':
                returned_index += 1
                (success, returned_index, returned_subtree) = Parameter(
                    returned_index - 1)  # check to see if index matches
                # parameter and if so increase index and check while statement
...
def Return(token_index):  # return function
    ...
    if success:
        ...
        return [True, returned_index - 1, subtree]  # return subtree
    return [False, token_index, []]
...
def Functionbody(token_index):  # functionbody check
    ...
    bodyfound = 0
    if tokens[
        token_index] != "RETURN":  # check to determine what path to take
        # depending on what token index equals
        bodyfound = 1
...
def Functionparam(token_index):  # check function param requirements
    if ("RPAREN" == tokens[
        token_index]):  # check to see if token is rparen and if
        # not fail
...
def FunctionDec(token_index):  # function declare function
    if 'FUNCTION' == tokens[
        token_index]:  # check various conditionals to make sure
        #  it matches function format
        token_index += 1
                    ...
                    if 'LBRACE' == tokens[token_index]:  # check token indexes
                        ...
                        if success:
                            if "RBRACE" == tokens[returned_index]:
                                subtree.append("LBRACE")
                                subtree.append(returned_subtree)
                                subtree.append("RBRACE")
                                return [True, returned_index, subtree]
    return [False, token_index, []]
...
def Namelist(token_index):  # Namelist Function
    ...
    if success:
        token_index += 1
        while tokens[
            token_index] == "COMMA":  # check each name object to make sure
            #  it matches syntax
            token_index += 1
            (success, returned_index, returned_subtree) = Name(token_index)
            if success:
...
def Assignment(token_index):  # Assignment function
    def SingleAssignment(token_index):
        if 'VAR' == tokens[token_index] and is_ident(
...
def Expression(token_index):  # check expression conditionals
    '''<Expression> ->
        <Term> ADD <Expression>
        | <Term> SUB <Expression>
        | <Term>
    ...
    (success, returned_index, returned_subtree) = Term(token_index)
    if success:
        return [True, returned_index,
                ["Expression2",
                 returned_subtree]]  # if all conditionals are
        # true then return subtree
    return [False, token_index, []]
...
def FunctionCallParams(token_index):
    ...
    if success:
        ...
        if "RPAREN" == tokens[returned_index - 1]:
            subtree.append(tokens[returned_index])
