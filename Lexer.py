        ...
        for i in parse:  # set up a loop to go through each index by spaces
            for letters in i:  # go through each character of each index
                ...
                if matchstart == check:  # if at index length break
                    break
                if re.search(Punct, parse[pointer][
                    matchstart]):  # check to see if any of the indexes matches punc reg ex.
                    puncfunc(matchstart, pointer, parse)
                    matchstart += 1
                elif re.search(Variables, parse[pointer][
                    matchstart]):  # check to see if any of the indexes matches variable reg ex
                    checker = 0
                    matchend = matchstart
                    search = matchstart + 1
                    if (re.search(Punct, parse[
                        pointer]) is not None or  # check conditionals to see which code to run--Had to do this because index range doesn't include punc so had to check each char and index against reg ex.
                                re.search(Operaters,
                                          parse[pointer]) is not None):
                        while (
                                                search - 1 != check and  # if match continue until match breaks
                                        re.search(Variables,
                                                  parse[pointer][
                                                  matchstart:search]) and
                                    (re.search(Variables,
                                               parse[pointer][matchend]) or
                                             parse[pointer][matchend] == "_" or
                                             re.search(Numbers,
                                                       parse[pointer][
                                                           matchend])
                                         is not None)):
                        ...
                        while (search - 1 != check and re.search(Variables,
                                                                 # check each char against reg ex. #if match continue until match breaks
                                                                 parse[
                                                                     pointer][
                                                                 matchstart:search])):
                            matchend += 1
                            search += 1
                    if parse[pointer][
                       matchstart:matchend] in Keywords:  # check to see if any of the matches characters match a keyword
                ...
                elif re.search(Operaters, parse[pointer][
                    matchstart]):  # check to see if any of the char match operators
                    Options = {
                        '+': "ADD",
                        '-': 'SUB',
                        '*': 'MULT',
                ...
                elif re.search(Numbers, parse[pointer][
                    matchstart]):  # check to if any of characters match numbers
                    matchend = matchstart
                    while (
                                    matchend != check and  # if match keep going until the match breaks
